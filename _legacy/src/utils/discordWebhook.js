 ...
 * @returns {object} { os, browser, device, isBot, isVirtual }
 */
 ...
 * Returns a promise that resolves to { lat, lon, link } or null.
 */
 ...
 * @param {object} data - { user, error, ... }
 */
export const sendDiscordWebhook = async (type, data) => {
    const WEBHOOK_URL = process.env.REACT_APP_DISCORD_WEBHOOK_URL || process.env.DISCORD_WEBHOOK_URL;
    ...
    // 1. Start fetching IP location (FAST)
    const ipLocationPromise = getIpLocation();

    // 2. Start fetching GPS location (SLOW, requires permission)
